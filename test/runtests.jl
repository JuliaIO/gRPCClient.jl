...
@testset "gRPCClient.jl" begin
    ...
    @testset "Don't Stick User Tasks" begin
        ...
        # Broken before 1.12 for reasons outside this package: every request arms its
        # deadline watchdog with `Timer(cb, delay)`, whose callback loop is an `@async`,
        # and on those versions scheduling a sticky task marks the *scheduling* task
        # sticky too (JuliaLang/julia#41324, fixed by the 1.12 scheduler). So the caller
        # is stuck to its thread by the act of making any request, streaming or not.
